...
{
    //事件更新
    public SingleEvent latestEvent; //最近事件
    ...
    private float _updateInterval = 1f;   //新事件产生间隔（选项可改动）
    private float _nextUpdate = 0.0f;    //下次更新的时间
    ...
    void Awake()
    {
        GameObject.Find("Name Text").GetComponent<Text>().text =  "名字：" + Abilities.myName;
    ...
    void Update()
    {
        ...
        if(Time.time > _nextUpdate)
        {
            ...
            if(Abilities.Health <= 0 || Abilities.Age >= Abilities.Lifespan) {  //将发生死亡事件
                //弹出文字提示死亡，点击进入结算界面
                SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex + 1);   //进入结算界面
            }
            else
            {  
                List<SingleEvent> latestEventList = EventManager.eventPool.FetchEvents(1);  //从事件池随机抽取 一个 新事件，以后可以拓展多事件
                
                if(latestEventList.Count != 0) 
                {
                    latestEvent = latestEventList[0]; 
                    EventManager.UpdateAbilities(latestEvent); //更新人物属性
                    Abilities.events.Add(latestEvent);
                    UpdateBar(); //更新下屏状态栏
    ...
    private void ShowNewEvent() //将latestEvent处理成卡片并显示，并右移所有eventCardList的位置
    {
        ...
        rt.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Left, 0, UpScreenHeight * latestEvent.widthRatio);
        rt.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Bottom, 0, UpScreenHeight * latestEvent.heightRatio);
        ...
        //如果要显示文字，则去掉注释
        latestEventCard.GetComponentInChildren<Text>().text = latestEvent.content;
    ...
    {
        switch(latestEvent.possibility)
        {
