...
public class UIAbilityInitializer : MonoBehaviour   //初始化界面使用的脚本类
{
    ...
    private List<SingleGift> giftsPresented;    //显示的天赋本体（肉）
    // private List<SingleGift> Abilities.gifts;
    ...
    private void Awake()
    {
        TestJSONProvider.Stimulate();   //本地生成JSON文件
        //NetRequests.QueryEvent();   //向网络发起数据库调用请求，加载事件池（实际上只有定期更新时调用）,暂时用TestJSONProvider本地生成代替！
        giftAvailableNum = Math.Min(giftAvailableNum, GiftManager.giftpool.poolCapacity);    //可选天赋数最大值不超过展示数量
        Abilities.gifts = new List<SingleGift>();
    ...
    private void SetGender(int option)
    {
        switch (option)
        {
            case 0:
                ...
                if (option == 1)
                {
                    Abilities.myGender = "男";
                }
                else if (option == 2)
                {
                    Abilities.myGender = "女";
                }
                else
                {
    ...
    {
        giftsPresented = new List<SingleGift>(GiftManager.giftpool.FetchGifts(giftPresentedNum));
        foreach(var item in giftObjectsPresented)    //重置表单
        {
            item.GetComponent<Text>().text = "";
        }
        for (int i = 0; i < Math.Min(giftPresentedNum, GiftManager.giftpool.poolCapacity); ++i)
        {
            giftObjectsPresented[i].GetComponent<Text>().text = giftsPresented[i].name + "：" + giftsPresented[i].detail;
        }
        HighlightChosenGift();
    }
    ...
    private void StartGame()
    {
        ...
        //若没有选择性别，则随机选择一个
        if (Abilities.myGender == "" || Abilities.myGender == null)
        {
            SetGender(0);
        }
        Abilities.Bear();
